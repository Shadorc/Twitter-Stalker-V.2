...
public class Stats {
	...
	public Stats(TwitterUser user, JButton bu, List <Status> statusList) throws TwitterException {
		...
		for(int i = 1; user.getTweetsAnalyzed() < tweetsToAnalyze; i++) {
			...
			System.out.println("User timeline : " + rls.getRemaining() + " / " + rls.getLimit() + ", unblocking in " + (rls.getSecondsUntilReset()/60) + "min " + (int) ((rls.getSecondsUntilReset()/60f - (int) (rls.getSecondsUntilReset()/60f))*60) + "s");
		...
		if(user.getName().equals(Frame.getTwitter().getScreenName())) {
			for(int i = 1; user.getMentionsAnalyzed() < OptionsPanel.getMaxMentionsNumber(); i++) {
				...
				System.out.println("Mentions timeline : " + rls.getRemaining() + " / " + rls.getLimit() + ", unblocking in " + (rls.getSecondsUntilReset()/60) + "min " + (int) ((rls.getSecondsUntilReset()/60f - (int) (rls.getSecondsUntilReset()/60f))*60) + "s");
